...
export const createRecording = async (req, res, next) => {
    try {
        const { sessionId, title, storageKey, accessMode, selectedUserIds = [], publishNow = false, durationDays = 7, autoTargetAbsent = false } = createRecordingSchema.parse(req.body);
        const now = new Date();
...
export const updateRecording = async (req, res, next) => {
    try {
        ...
        const { selectedUserIds, durationDays, ...data } = updateRecordingSchema.parse(req.body);
...
export const updateRecordingAccess = async (req, res, next) => {
    try {
        const id = req.params.id;
...
export const publishRecording = async (req, res, next) => {
    try {
        const id = req.params.id;
...
export const getRecordingPlayback = async (req, res, next) => {
    try {
        const id = req.params.id;
