...
export const registerStudent = async (data: RegisterInput) => {
  ...
  if (existingPhone) {
    const error: any = new Error("رقم الهاتف (واتساب) مسجل بالفعل، يمكنكِ تسجيل الدخول مباشرة");
  ...
  if (email) {
    ...
    if (existingEmail) {
      const error: any = new Error("البريد الإلكتروني مسجل بالفعل لحساب آخر");
      error.statusCode = 400;
      throw error;
    }
  }
...
export const loginUser = async (data: LoginInput) => {
  ...
  if (!identifier) {
    const error: any = new Error("يرجى إدخال رقم الهاتف أو البريد الإلكتروني");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (!user) {
    const error: any = new Error("رقم الهاتف أو كلمة المرور غير صحيحة");
  ...
  if (user.status !== "ACTIVE") {
    const error: any = new Error("حسابكِ قيد المراجعة أو غير مفعل من المعلمة بعد");
...
export const requestPasswordReset = async (data: ForgotPasswordInput) => {
  ...
  if (!user) {
    const error: any = new Error("رقم الهاتف غير مسجل لدينا، يرجى التأكد من الرقم المدخل");
...
export const verifyResetCode = async (data: VerifyResetCodeInput) => {
  ...
  if (!resetRecord) {
    const error: any = new Error("رمز التحقق غير صحيح، يرجى التأكد من الرمز المدخل");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (new Date() > resetRecord.expiresAt) {
    ...
    const error: any = new Error("انتهت صلاحية رمز التحقق (30 دقيقة)، يرجى طلب رمز جديد");
    error.statusCode = 400;
    throw error;
  }
 ...
 */
export const executePasswordReset = async (data: ResetPasswordInput) => {
  const phoneInput = data.phone?.trim();
  const emailInput = data.email?.trim().toLowerCase();
  ...
  if (!resetRecord) {
    const error: any = new Error("طلب استعادة كلمة المرور غير صالح أو تم استخدامه من قبل");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (new Date() > resetRecord.expiresAt) {
    await prisma.passwordReset.delete({ where: { id: resetRecord.id } });
    const error: any = new Error("انتهت صلاحية طلب الاستعادة، يرجى تقديم طلب جديد");
    error.statusCode = 400;
    throw error;
  }
  ...
  if (!user) {
    const error: any = new Error("المستخدم غير موجود");
    error.statusCode = 404;
    throw error;
  }
